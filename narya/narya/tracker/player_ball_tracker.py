...
class PlayerBallTracker:
    ...
    def get_tracking(
        ...
        for image in imgs:
            ...
            results.append((frame_id + 1, avg_colors, online_boxs, online_ids))
            if save_tracking_folder is not None:
                ...
                for idx, box in enumerate(online_boxs):
                    color = avg_colors[idx]

                    if not np.isnan(color).any():
                        cv2.rectangle(im_box,(int(box[0]),int(box[1])),(int(box[2]),int(box[3])),color,3)
